...
+ **O 應該** 使用明確的參數名字來表示較少參數的多載所使用的預設值。  
    ...
    ```
    以上面例子而言，第二個多載應該命名為 `ignoreCase` (預設 false) 而不是 `caseSensitive` (預設 true)，因為 Boolean 的預設值是 false。  
...
+ **O 應該** 只能讓擁有最多參數的成員作為虛擬成員使用 `virtual` 關鍵字。  
    ...
    public class String
    {
        ...
        {
            return IndexOf(s, startIndex, s.Length - startIndex);
        }
    ...
    CHRIS SELLS 表示這個規則除了比較好寫，測試時也不用測試所有的多載。另外如果有兩個多載，他總是讓較少參數的多載呼叫次少參數的多載而不是最多參數的多載，他不會在多個多載中重複填寫各參數的預設值，如果發生這種事可能就是寫錯了。

    > 總之就是：  
    > 1. 關於多個多載之間的呼叫順序，依序呼叫能將維護時混淆的風險降到最低。  
...
+ **O 應該** 優先使用多載而不是預設引數。  
    ...
    BRAD ABRAMS 表示這不是鼓勵將 `null` 作為 magic constant，而是用來避免額外的檢查，事實上在呼叫 API 時傳遞 `null` 作為引數，通常代表弄錯了什麼或是 API 的多載設計不良。
...
+ **X 避免** 明確實作介面成員，除非有強烈的理由。  
    ...
    > 因為呼叫端呼叫明確實作介面的成員時，需要向上轉型成介面後才能呼叫，當該型別是值型別時就會在向上轉型時造成裝箱。  
...
> 這一節比較像是在說 "操作資料/欄位" 這個議題上要選擇屬性還是方法。  

RICO MARIANI 表示屬性操作上看起來很像欄位，呼叫端會直接存取他而不會希望存取屬性的效能比欄位差太多，同時也不會預期每次取得的屬性值不固定。

JOE BUFFY 表示屬性應該只包含少量程式碼，所以避免 `if` 條件式、`try-catch`、呼叫其他方法等，並且只包含存取欄位的內容，這樣就能避免 Rico 說的效能問題。  
...
+ **O 考慮** 使用屬性來表示型別中的一項特徵。  
    例如： `Button.Color` 設計成屬性是因為顏色是按鈕的一個特徵。  
    > 或是說 "特徵" 比較貼切。
...
+ **O 應該** 使用方法而非屬性的情境有：  
    ...
    - 當該操作有明顯的副作用時。要注意的是操作內部快取 (Internal Cache) 不算副作用。  
        ...
        BRIAN PEPIN 表示 Windows Forms 控制項就是太多副作用的典型案例。存取 `Handle` 屬性會建立原本還沒建立的控制代碼 ([Handle](https://learn.microsoft.com/en-us/dotnet/api/system.windows.forms.control.handle))，經常因為在除錯過程中讀取 `Handle` 屬性而改變了 debugging session，進而隱藏了 bug。  
    ...
    - 當該操作回傳陣列時。  
        ...
        因應方式有兩種。一是改成方法，讓呼叫端明確知道這個呼叫不是直接取得內部資料；二是回傳唯讀集合 (`ReadOnlyCollection<T>` 之類的) 型別的物件，避免被外部改變內部資料的同時又不需要複製陣列。  
        BRAD ABRAMS 表示這個回傳陣列的屬性所造成的問題是 .NET Framework 1.0 時候真實遇到的，數以千計的大量陣列都在建立後段時間內就廢棄回收，才因此訂立了這個規則。  

RICO MARIANI 表示這所有規則都很重要，能避免很多嚴重的問題。整體可以簡化成一個口訣：**屬性用於單純存取簡單資料或簡單的運算**。不要偏離這個核心。  
...
+ **O 應該** 在不允許呼叫端變更屬性值時使用唯讀屬性。  
    ...
    > 如果真的要嚴格禁止呼叫端變更屬性值與內容，那就應該使用 (或自製) 唯讀型別，例如：`ReadOnlyDictionary<TKey,TValue>`、`ReadOnlyCollection<T>` 等。
...
+ **X 避免** 讓索引子的參數在 `System.Int32`、`System.Int64`、`System.String`、`System.Object`、列舉 (Enum) 之外。  
...
+ **O 考慮** 當高階 (high-level) API 的屬性變更時發起變更通知事件。
    雖然是這樣建議，但是如果是低階 (low-level) 的 API (例如 `List<T>`) 就不應該因為屬性變更而發起事件。
...
+ **O 應該** 透過建構子參數來設定主要的屬性。   
    ...
    ``` csharp
    var ron = new Person();
    // User unfriendly, caller should remember to initialize main properties after constructing an instance of Person.
    ron.Name = "Ron";
...
+ **O 應該** 在類別中明確寫出公開預設建構子 (如果需要的話)。
    ...
    這點不適用於結構，因為即使有其他建構子，編譯結構時仍然會預設加入預設建構子。
...
+ **X 避免** 在建構子中呼叫虛擬成員。
    ...
    new Derived3();
    /* Output:
    Derived3.AbstractMethod() called
    Derived3.VirtualMethod() called
    Base constructed
    Derived1 constructed
    Derived2 constructed
    Derived3 constructed
    */
    ```
    > 順序是：
    > 1. 最頂層建構子。
...
#### 型別建構子的設計
型別建構子又稱靜態建構子 (Static Constructor)，CLR 會在第一次建立此型別的執行個體之前，或第一次存取其任何靜態成員之前呼叫靜態建構子。
...
+ **O 考慮** 直接初始化靜態欄位，而不是明確使用靜態建構子，因為當型別沒有明確定義的靜態建構子時，CLR 可以最佳化該型別的效能。
    ``` csharp
    // unoptimized code
    public class Foo
    {
...
+ **O 應該** 在發起靜態 (Static) 事件時傳入 `null` 作為 sender。
    > ChatGPT 說的：兩個規則差別在於靜態事件和實例 (Instance) 無關，所以 sender 應該是 `null`，但非靜態事件和實例息息相關，所以不應該傳入 `null`。  
...
歸納所有規則，總之事件處理常式應該長這個形狀 `public delegate void MyEventHandler(object sender, MyEventArgs e);`，幾乎所有地方都要一模一樣，除了以下幾個地方：
...
+ **X 不應** 將可變 (Mutable) 型別設計成唯讀欄位。
    ...
    ```
    > 屬性也一樣，如果要達到 "真唯讀" 就應該使用 `ReadOnlyCollection<T>`、`ReadOnlyDictionary<TKey,TValue>` 等確定內容不可變的型別。
...
+ **O 考慮** 提供與運算子多載對應的方法。  
    因為很多語言不支援運算子多載，所以要提供 `Subtract` 方法來對應 `-` 運算子。
...
> 就 `implicit` (隱含轉換運算子) 和 `explicit` (明確轉換運算子)
