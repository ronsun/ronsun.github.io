...
+ 參考型別 (Reference Types)
  ...
  - 特性 (Attributes)
+ 值型別 (Value Types)
  -  結構 (Structs)：值型別中常見的形式，應該保留給像是語言原生型別 (Primitive Types) 這種小型簡單的型別。
  -  列舉 (Enums)  
...
> 仔細想想以往應用程式的設計上經常犯這個錯誤，實務上的確也引起了不小的混淆與使用上的困擾，更麻煩的是，這可能是因為早期命名空間規劃不當而間接引起的，要處理還得改動命名空間的設計。
...
JEFFRY RICHTER 表示有一個範例：`System.SerializableAttribute` 和 `System.NonSerializedAttribute` 因為收納在不恰當的命名空間下，使得使用者沒意識到它們應該和 `System.Runtime.Serialization` 命名空間下的型別搭配使用，而經常誤將它們和 `System.Xml.Serialization` 下的 `XmlSerializer` 搭配使用，實際上 `System.SerializableAttribute` 和 `System.NonSerializedAttribute` 搭配 `System.Xml.Serialization.XmlSerializer` 使用時是沒有作用的。  
...
+ **X 不應** 建立沒有命名空間的型別。
    > 作者群補充了不少關於命名空間和編譯器與 CLR 之間的關係與運作機制來附註說明這個原則，內容有點多且這個原則也很理所當然，所以就不記錄了。
...
> 看起來像是和舊元件之間的隔離層，讓上層能不用去和舊元件互動。但是這邊太多看不懂的東西了，實在不知道怎麼整理，先留個 {% asset_link "PIA.pdf" "ChatGPT 的問答紀錄" %} 之後再回來看。

## 選擇使用類別或是結構
要做出適當選擇就要先了解他們之間的差異。  

首先是記憶體配置與回收，類別是參考型別，所以物件本身通常配置在堆積 (Heap) 中且受到垃圾回收 (GC) 機制管理與回收記憶體；而結構則不同，它可能配置在堆疊 (Stack) 中，也可能因為被其他型別包含而隨著包含著它的物件被配置在堆疊或堆積中，回收時機則是在堆疊回溯 (unwind) 時或是包含著它的物件被回收時。  
...
> C# 8 中的預設介面方法 (Default Interface Methods) 的出現，其中一個原因就是為了降低已存在的介面擴充時的困擾 ([Issues 52](https://github.com/dotnet/csharplang/issues/52)、[Proposal](https://github.com/dotnet/csharplang/blob/main/proposals/csharp-8.0/default-interface-methods.md))。  
...
+ **O 考慮** 在已經有繼承其他類別的時候提供介面來提供共用功能。 
  > 但我還是傾向遵守 JEFFREY RICHTER 的建議，也就是繼承是 is-a 的關係，而介面則是 can-do 的關係。不管是因為什麼原因而使用介面，設計時仍然要符合這樣的定義。

+ **X 避免** 使用標記用的介面，也就是沒有成員的空介面。  
  如果需要的話應該使用特性 (Attribute) 來達到標記的效果。  

  RICO MARIANI 表示的確偵測特性 (Attribute) 比確認介面型別的成本還要高很多，所以基於效能還是可能會需要標記用的空介面，只是以他自己的經驗來說這種事不常見。

  這個想法的問題在於特性 (Attribute) 只能在執行階段確認，有時候必須在編譯時期就確認型別時，就可以接受使用標記用的型別，例如下面的範例：  
  ``` csharp
  // empty interface
...
+ **X 不應** 在已發布出去的介面上新增成員。  
  這樣實作的類別會壞掉，所以通常只能建立新的介面來避免升版造成的問題，這也是之前提到應該優先使用抽象類別而不是介面。
...
有幾個原因：  
1. 值型別賦值時是建立另外一個物件而不是原物件位址，開發者可能不會注意到他們修改屬性的時候修改的內容已不是原物件。
  > 如範例：  
    ``` csharp
    public struct Point
    {
...
+ **X 不應** 把開放性的集合 (Open Sets) 定義成列舉，例如作業系統的版本或是朋友的名字。
  > 應該是因為這些值是不是固定的，一旦需要增加就需要改程式並重新編譯程式碼，這類資料定義成字串或陣列等型別會比較穩定 (畢竟是對於框架設計的建議所以比較嚴格)。
...
+ **O 應該** 提供預設值 0 作為列舉的預設值。
  ...
  > 我認為是因為值型別預設值是 0，如果 0 有意義的話呼叫端其實很難判斷這個情境是沒處理好所以是預設值還是真的想要使用 0。因此將 0 定義成 "不適用" 是非常恰當的慣例。
...
BRAD ABRAMS 表示發布後改列舉基礎型別是 [二進位中斷性變更 (Binary Breaking Change)](https://learn.microsoft.com/en-us/dotnet/standard/library-guidance/breaking-changes#binary-breaking-change)，所以要謹慎考慮未來再做決定。以他們的經驗來說 `Int32` 通常都是正確的選擇，所以他們才會用其作為預設。  

> 基本上就是用預設，除非是非常必要且不會再次變更的時候再考慮用其他基礎型別。

+ **X 不應** 直接擴充 `System.Enum`。
  它是 CLR 為了建立自訂列舉 (User-defined Enumerations) 而提供的特殊型別，多數語言都提供相關的語法元素讓我們直接使用，例如：C# 中的 `enum`。
...
+ **X 避免** 把某些組合不合理的情境設計成旗標列舉。  
  ...
  > 以下面為例，雖然車輛不會同時是摩托車 (`VehicleType.Motorbike`) 又是腳踏車 (`VehicleType.Bicycle`) ，但是在用來判斷 "包含" (這台車是不是兩輪車) 的時候也很好用，例如：  
    ...
    {
        return VehicleType.TwoWheelers.HasFlag(vehicle.Type)
    }
    ```
  > 
  > 甚至要用在搜尋的情境也比集合很有效率，例如：
    ...
    {
        return discountVehicleTypes.HasFlag(vehicle.Type);
    }
...
+ **O 應該** 在希望或必須讓內層類別需要存取外層類別的私有成員時使用巢狀型別。
  例如：
  ``` csharp
  public class OrderCollection : IEnumerab1e 
  {
      Order[] data = ...;
  ...
  > 附上 ChatGPT 完成的程式碼如下 (用於概念示範所以不考慮細節的正確性)：
    ...
    ```
  > 
  > 我實務上會用到巢狀型別的情境還有複雜的 Model，有些巢狀的 Model 的內層型別非常特殊且完全無法共用，這時候把它收在內層能讓維護人員認知到內層型別只適用於該外層，範例如下：  
    ``` csharp
    public class Order
    {
...
+ **X 避免** 把巢狀型別公開 (Publicly Exposed)，除非在非常少見的情況下，它真的需要被其他呼叫端使用時。
...
+ **X 不應** 將會由外部程式碼初始化的類別定義成巢狀型別，如果一個類別有公開級別的建構子 (Public Constructor)，那它就不應該是巢狀型別。
  > 但在應用層的專案中經常用到的資料容器型的類別 (例如：DTO)，即使會被外部程式碼初始化，只要定義上符合 "只適合它的外層類別" 我還是會考慮使用巢狀型別。

+ **X 不應** 將巢狀型別定義成介面的成員。

一般來說，巢狀型別使用上應該非常的保守，且避免將存取權限設為公開 (Public)。
  > 就我的理解，巢狀型別存在的情境是 "只適合它的外層類別"，所以可被存取範圍應該限縮在外層類別內。  
...
> 平常碰得不夠底層，這一節除了資訊揭露相關型別的部分外，我是幾乎看不懂，雖然知道怎麼用，但就是不知道為什麼以及有什麼好處。
